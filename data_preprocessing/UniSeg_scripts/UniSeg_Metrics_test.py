...
gt_path = save_path.replace("Predict", "mask")
isHD = False
print(save_path)
...
def evaluation(root, i):
    ...
    i2_file = os.path.join(gt_path, i.split(".")[0]+"_0000.nii.gz")
    ...
    sub_class = 1
    dice_sub_class = dice_score(np.logical_or(pred == sub_class, pred == 2), label == 4)
    print(dice_sub_class)
    if isHD:
        HD_sub_class = compute_HD95(np.logical_or(pred == sub_class, pred == 2), label == 4)
    else:
        HD_sub_class = 999.
